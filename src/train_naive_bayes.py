...
def main(dataset):
    ...
    plt.savefig(f"{models_dir}/confusion_matrix_nb.png", dpi=150, bbox_inches="tight")
    plt.close()

    joblib.dump(model, f"{models_dir}/naive_bayes_model.joblib")
    joblib.dump(
        {"model": "Naive Bayes", "accuracy": acc, "precision": prec, "recall": rec, "f1": f1},
        f"{models_dir}/naive_bayes_metrics.joblib"
    )
